...
@section('content')
    @component('components.tabs',['title'=>'文章配置'])
        @slot('nav')
            <br>
            <a href="/article/category" class="btn btn-primary"><i class="fa fa-plus"></i> 栏目列表</a>
        ...
        @slot('body')
            <div class="panel-body">
                <form class="form-horizontal" method="post" action="/article/category/{{$category->id}}">
                    @csrf
                    @method("PUT")
                                ...
                                <div class="col-sm-10">
                                    <select name="pid" class="js-example-basic-single form-control">
                                        ...
                                        @foreach($data as $key =>$v)
                                            <option value="{{$v['id']}}"  {{$v['_selected']?'selected':''}}
